...
- (void)viewDidLoad
{
   ...
   bg = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 320, 320)]; // make an imageview on the scroll
    ...
    bg.image = [UIImage imageNamed:@"mzl.pgzonqif.png"]; // sequential images
    ...
    sv.contentSize = CGSizeMake(320, (self.view.bounds.size.height*2)-130); // make it's content size bigger than the screen // adjust the variable to make sure there's no whitespace at the botton
    ...
    UIImageView *imv = [[UIImageView alloc] initWithFrame:CGRectMake(0, 330, 320, self.view.bounds.size.height)]; // make an imageview on the scroll
    imv.contentMode = UIViewContentModeScaleAspectFill; // because it's content mode is this, we don't need to worry about height
    imv.image = [UIImage imageNamed:@"m4@2x.png"]; // sequential images
